...
fn fork_release_is_windows_only_immutable_and_tmux_bound() {
    ...
    assert!(
        workflow.starts_with("# PERSONAL FORK:"),
        "fork-only workflow must be visibly marked at the top"
    );
    for required in [
        "v*-jukrb0x.*",
        "workflow_dispatch:",
...
#[test]
fn manual_dispatch_validates_current_tmux_sha_without_publishing() {
    let workflow = repo_file(".github/workflows/windows-fork-release.yml");
    let jobs = indented_block(&workflow, "jobs:", 0);
    let build = indented_block(&jobs, "build:", 2);
    let steps = indented_block(&build, "steps:", 4);
    let checkout = indented_block(
        &steps,
        "- uses: actions/checkout@34e114876b0b11c390a56381ad16ebd13914f8d5",
        6,
    );
    let validation = indented_block(&steps, "- name: Validate release event and source", 6);
    let publish = indented_block(&steps, "- name: Publish immutable fork release", 6);

    assert!(checkout.contains("ref: ${{ github.sha }}"));
    assert!(!checkout.contains("inputs."));
    ...
    for required in [
        ...
        "if ($env:GITHUB_REF -ne \"refs/heads/tmux\")",
        "if ($head -ne $tmuxCommit)",
        "$buildId = $head",
        ...
        assert!(
            ...
            "release event validation lost required behavior: {required}"
        );
    }

    assert!(workflow.contains("name: rmux-${{ env.BUILD_ID }}-windows-x86_64"));
    assert!(publish.contains("if: github.event_name == 'push' && github.ref_type == 'tag'"));
    assert!(publish.contains("gh release create"));
    assert!(!validation.contains("inputs.tag"));
...
fn inherited_release_push_trigger_excludes_personal_tags_after_the_public_pattern() {
    ...
    let public_position = push.find("- \"v*\"").expect("public tag pattern");
    let marker_position = push
        .find("# PERSONAL FORK:")
        .expect("personal fork trigger marker");
    let exclusion_position = push
        .find("- \"!v*-jukrb0x.*\"")
        .expect("personal tag exclusion");
    assert!(public_position < marker_position && marker_position < exclusion_position);
}
...
fn inherited_release_rejects_personal_tags_before_source_work() {
    ...
    assert!(source_gate_env.contains(
    ...
    assert!(!rejection_gate.contains("^v[0-9]+\\.[0-9]+\\.[0-9]+-jukrb0x\\.[1-9][0-9]*$"));
    ...
    for reserved in [
        ...
        assert!(is_reserved_fork_tag(reserved));
    }
    assert!(!is_reserved_fork_tag("v0.9.0"));
    ...
    assert!(rejection_position < validate_position && rejection_position < checkout_position);

    for job in ["build:", "publish:"] {
        let job = indented_block(&jobs, job, 2);
        let needs = indented_block(&job, "needs:", 4);
        assert!(needs.lines().any(|line| line.trim() == "- source-gates"));
    }
}

#[test]
fn inherited_ci_and_scorecard_target_tmux_and_preserve_main() {
    ...
    assert_eq!(
        ...
        ["- \"v*\"", "- \"!v*-jukrb0x.*\""],
        "upstream CI tags must exclude personal release tags"
    );
    assert_eq!(
        indented_list(&ci_push, "branches:", 4),
        ["- main", "- tmux"],
        "CI push must preserve upstream main and target the fork default branch"
    );
    assert_eq!(
        indented_list(&ci_pull_request, "branches:", 4),
        ["- main", "- tmux"],
        "CI pull requests must preserve upstream main and target the fork default branch"
    );
    assert_eq!(
        indented_list(&scorecard_push, "branches:", 4),
        ["- main", "- tmux"],
        "Scorecard push must preserve upstream main and target the fork default branch"
    );

    for workflow in [&ci, &scorecard] {
        assert!(workflow.contains("# PERSONAL FORK:"));
    }
}
